...
def get_features_from_h5(h5_path: Path, device: torch.device):
    ...
    if feats_np.size == 0:
        raise ValueError(f"[H5] empty bag for in {h5_path}")
...
def run_pipeline_single_slide(cfg_path: str, input_slide: Path, save_jpg: bool):
...
def main():
    ...
    run_pipeline_single_slide(cfg_path=args.config, input_slide=input_slide, save_jpg=args.save_jpg)
